...
export function validateConfig() {
  ...
  if (!config.basescan.apiKey) console.warn('⚠️  BASESCAN_API_KEY not set — wallet & dev tracker disabled')
  if (!config.github.token) console.warn('⚠️  GITHUB_TOKEN not set — GitHub rate limit: 60 req/hour')
  if (!config.telegram.botToken) console.warn('⚠️  TELEGRAM_BOT_TOKEN not set — Telegram bot disabled')
  if (!config.telegram.channelId) console.warn('⚠️  TELEGRAM_CHANNEL_ID not set — auto-notifications disabled')
  if (!config.flaunch.apiKey) console.warn('⚠️  FLAUNCH_API_KEY not set — get one at builders.flaunch.gg')
